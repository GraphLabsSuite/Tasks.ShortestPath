...
*/
 ...
 /* graphMy(data:any): IGraph<IVertex, IEdge> {
  ...
  get_graph(): IGraph<IVertex, IEdge>{
    ...
    edges.forEach((e:any) => {
      if (e.label){
        data[0].value.edges.push({"source": e.vertexOne, "target": e.vertexTwo, "weightLabel": e.label})
      }
      else{
        data[0].value.edges.push({"source": e.vertexOne, "target": e.vertexTwo, "weightLabel": Math.round(Math.random()*10).toString()})
      }
    });
